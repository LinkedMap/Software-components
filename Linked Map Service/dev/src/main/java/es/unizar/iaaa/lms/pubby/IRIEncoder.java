...
public class IRIEncoder {
    ...
    /**
     * Converts a URI to an IRI by removing unnecessary percent-encoding of
     * UTF-8 sequences.
     */
    ...
    private static final Pattern percentEncoding = Pattern
        .compile("(%[0-9a-fA-F][0-9a-fA-F])+");

    /**
     * Converts an IRI to a URI by percent-encoding characters outside of the
     * US-ASCII range.
     */
    ...
    private static int[] toBytes(String percentEncoded) {
        ...
        for (int i = 0; i < length; i++) {
            result[i] = toByte(percentEncoded.charAt(i * 3 + 1),
                percentEncoded.charAt(i * 3 + 2));
        }
        return result;
    }
