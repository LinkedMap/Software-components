...
import es.unizar.iaaa.lms.pubby.vocab.META;
...
public class MetadataConfiguration extends ResourceReader {
    ...
    public void addCustomMetadata(Model document, Resource documentResource) {
        ...
        it = customTemplate.getModel().listStatements(null, null,
            customTemplate);
        while (it.hasNext()) {
            Statement stmt = it.nextStatement();
        ...
        while (it.hasNext()) {
            ...
            try {
                if (subj.toString().contains(metadataPlaceholderURIPrefix)) {
                    subj = (Resource) parsePlaceholder(subj, controller,
                        currentTime, currentDocRepr);
                    if (subj == null) {
                        // create a unique blank node with a fixed id.
                        subj = getModel().createResource(
                            new AnonId(String.valueOf(stmt.getSubject()
                                .hashCode())));
                    }
                }

                if (obj.toString().contains(metadataPlaceholderURIPrefix)) {
                    obj = parsePlaceholder(obj, controller, currentTime,
                        currentDocRepr);
                }
    ...
    private RDFNode parsePlaceholder(RDFNode phRes,
            HypermediaControls controller, Calendar currentTime,
            Resource currentDocRepr) {
        ...
        if (phPackage.equals("runtime")) {
            ...
            if (phName.equals("query")) {
                if (sparqlDataSource == null)
                    return null;
                return getModel().createTypedLiteral(
                    sparqlDataSource.getPreviousDescribeQuery());
            }
        ...
        if (phPackage.equals("config")) {
            ...
            StmtIterator it = dataset.getModel().listStatements(null,
        ...
        return getModel().createResource(
            new AnonId(String.valueOf(phRes.hashCode())));
    }
}
