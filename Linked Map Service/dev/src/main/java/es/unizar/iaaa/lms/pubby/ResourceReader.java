...
/**
 * Convenience superclass for classes that read properties of an RDF resource,
 * intended for making the reading of configuration files easier.
 */
public abstract class ResourceReader {
    ...
    public void requireExactlyOneOf(Property... properties) {
        ...
        for (Property p : properties) {
            if (resource.hasProperty(p)) {
                if (found == null) {
                    ...
                    throw new PubbyRepositoryException("Can't have both "
        ...
        if (found == null) {
            throw new PubbyRepositoryException("One of " + s.toString()
    ...
    private void assertHasOneValue(Property p) {
        StmtIterator it = resource.listProperties(p);
        if (!it.hasNext()) {
            throw new PubbyRepositoryException("Missing property " + pretty(p)
                    + " on resource " + pretty(resource));
        }
        it.next();
        if (it.hasNext()) {
            throw new PubbyRepositoryException("Too many values for property "
    ...
    private void assertResourceValue(Statement stmt) {
        if (stmt.getObject().isLiteral()) {
            throw new PubbyRepositoryException(
    ...
    private void assertLiteralValue(Statement stmt) {
        ...
        if (stmt.getObject().isAnon()) {
            throw new PubbyRepositoryException(
                "Expected literal object, found blank node: "
                        + pretty(stmt));
        }
    }
    ...
    private void raiseUnexpectedDatatype(String expectedDatatype, Statement stmt) {
        throw new PubbyRepositoryException("Expected " + expectedDatatype
                + " object, found other datatype: " + pretty(stmt));
    }
}
