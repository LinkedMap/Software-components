...
/**
 * The hypermedia interface to a specific resource. The resource may be hosted
 * by the server or merely made browsable by the server.
 */
public class HypermediaControls {
     ...
     * 
     * @param path
     *            A Pubby path, either absolute or relative
     * @param config
     *            The server's configuration
     * @return An object providing hypermedia controls for the resource, or null
     *         if this is not a Pubby-browsable resource.
     */
    public static HypermediaControls createFromPubbyPath(String path,
        ...
        try {
            if (new URI(path).isAbsolute()) {
                ...
                return createFromIRI(
                    config.getWebApplicationBaseURI()
     ...
     * 
     * @param absoluteIRI
     *            An IRI identifying the resource
     * @param config
     *            The server's configuration
     * @return An object providing hypermedia controls for the resource, or null
     *         if this is not a Pubby-browsable resource.
     */
    public static HypermediaControls createFromIRI(String absoluteIRI,
            PubbyRepository config) {
        if (absoluteIRI == null)
            return null;
        if (!config.isBrowsable(absoluteIRI))
            return null;
    ...
    private final PubbyRepository config;

    private HypermediaControls(String absoluteIRI, PubbyRepository config) {
        this.absoluteIRI = absoluteIRI;
        this.config = config;
        this.isHosted = absoluteIRI.startsWith(config
            .getWebApplicationBaseURI());
    }

    /**
     * A version of the resource's IRI suitable for use in constructing Pubby
     * path URLs. If the resource is Pubby-hosted, this will be its IRI relative
     * to the resource base (<code>conf:webBase + conf:webResourcePrefix</code>
     * ). Otherwise, it will be its full absolute IRI with special characters
     * escaped to make it safe in constructing paths.
     * 
     * @return
     */
    ...
    /**
     * Is the resource itself hosted by Pubby, that is, is it in Pubby's web
     * base namespace? (If not, then Pubby merely provides a hypermedia browsing
     * interface for the resource, but cannot be said to host the resource
     * itself.)
     */
    ...
    /**
     * @return A description of the resource that allows interrogation of the
     *         data
     */
    public ResourceDescription getResourceDescription() {
        ...
        return new ResourceDescription(this, model,
            ...
            source.getHighOutdegreeProperties(absoluteIRI), config);
    }
    ...
    public String getDataURL() {
        return config.getWebApplicationBaseURI() + "doc/" + getPubbyPath()
                + ".ttl";
    ...
    private String getPathURL(String urlPrefix, Property property) {
        if (config.getPrefixes().qnameFor(property.getURI()) == null) {
            String encoded = PubbyIRIEscaper.escapeSpecialCharacters(property
                .getURI());
            return config.getWebApplicationBaseURI() + urlPrefix + "!"
                    + encoded + "///" + getPubbyPath();
        }
        return config.getWebApplicationBaseURI() + urlPrefix
